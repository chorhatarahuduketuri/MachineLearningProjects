...
##### Step four
The fourth step is to 'consider what sorts of models would be appropriate, as well as understand which sorts of algorithms will work and which will not. Feature engineering should also be undertaken at this stage, in the case of any selected model that would benefit from or require it.'\
As usual, I will use the simplest to implement and most common of algorithms. Since it's a regression problem, that'll be linear regression. As per usual, I'll also use SVMs. Should I use ANNs? I won't, because so far their performance hasn't been as good. Besides, maybe I could use something like naive bayes, if the other two don't make good enough predictions?\
Also as usual, I will be running each algorithm with both the original training data, and 2nd order polynomial training data generated by the `PolynomialFeatures` import from the `sklearn.preprocessing` library. 
...
'Evaluation of model on Validation set'
###### Logistic Regression
On 1st order training data, logistic regression has an absolute error of between 3 and (just under) 4. This means it's estimates are $3000-$4000 off, out of a range of $45000 (max $50000, min $5000). That's not amazing, in terms of real world accuracy, and it's not totally awful. \
With 2nd order training data, logistic regression varied between (an observed) 2.33 and 7.74. So somewhere between $2330 and $7740 off the real price. Which is slightly better on a good day, and unusable on a bad day. 

##### Support Vector Machines
Using 1st order training data, SVMs gave absolute mean errors of 5.38 to 6.96. That entire range is, compared to the total range of the prices of houses in that area, totally useless. \
When provided with 2nd order training data, SVMs 3.42 4.98. Also unusably inaccurate. 

Unfortunately, the first dataset where I have any realistic sense of how much the error matters, I've found that my errors are relatively huge. Which sucks. I suppose I'll have to consider using ANNs. 

##### Step seven - take 1
'Hyperparameter turning (improving model performance). Algorithm tuning, ensemble methods.'\
I'm not going to engage in any hyperparameter tuning, because I didn't set any parameters. However, I will be trying ANNs on this, because I said I would if the results weren't good enough. 

##### Step five - take 2
'Design, create, and/or train the model'
I implemented two ANNs, one for 1st order training data and one for 2nd order training data.\
The ANN for 1st order training data had 2 hidden layers, each with 13 neurons. This is because there are 13 features, and it's (apparently) a good idea to have at least as many neurones per layer as there are features. I also set the max_iter to 5000 to remove any lack of convergence problems.\
The ANN for the 2nd order training data had 2 hidden layers as well, this time with 105 neurons each, due to there being 105 features in the 2nd order polynomial training dataset.

##### Step six - take 2
'Evaluation of model on Validation set'
This time I decided to record some numbers and put them into a spreadsheet (because who doesn't love practicing the use of those) and gather some data (because that's the new religion, right?)\
I ran the code 16 times and used the results. 

| Statistical Attribute | LR1 | LR2 | SVM1 | SVM2 | ANN1 | ANN2 |
| --- | --- | --- | --- | --- | --- | --- |
| Mean | 3.46 | 2.65 | 3.28 | 4.23 | 2.48 | 2.58 |
| Minimum | 3.07 | 2.34 | 2.34 | 3.48 | 2.14 | 2.29 |
| Maximum | 3.73 | 2.96 | 4.32 | 5.35 | 2.91 | 3 |

As you can clearly see, the performance of the ANNs was actually better for once. They have _both_ the lowest mean scores overall, as well as the lowest minimums, and 2 out of the 3 lowest maximums.
